...
class Matrix:
    ...
    def __ne__(self, v):
        return not self == v  # operator!=

    def __eq__(self, v):
        if self.cols != v.cols:
            return False
        for i in range(self.cols):
            if self.columns[i] != v.columns[i]:
                return False
        return True
    ...
    def rc(self, row, col):
        return self.columns[col].idx(row)
    ...
    def determinant(self):
        ...
        if self.rows == 1:
            ...
            return rc(0, 0) * rc(1, 1) - rc(1, 0) * rc(0, 1)
        elif self.rows == 3:
            return (
                rc(0, 0) * (rc(1, 1) * rc(2, 2) - rc(1, 2) * rc(2, 1))
                - rc(0, 1) * (rc(1, 0) * rc(2, 2) - rc(1, 2) * rc(2, 0))
                + rc(0, 2) * (rc(1, 0) * rc(2, 1) - rc(1, 1) * rc(2, 0))
            )
        else:
            raise NotImplementedError("determinants for 4x4 matrices and above not yet implemented")

    def minors(self):
        raise NotImplementedError("minors not implemented yet")

    def covariant(self):
        raise NotImplementedError("covariant not implemented yet")

    def adjugate(self):
        return self.covariant().transpose()
