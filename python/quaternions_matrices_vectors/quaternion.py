...
class Quaternion:
    ...
    def __str__(self):
        s = "Quat("
        if self == Quaternion.i():
            return s + "i)"
        if self == Quaternion.j():
            return s + "j)"
        if self == Quaternion.k():
            return s + "k)"
        if self.magnitude() == 0.0 and self.w == 0:
            return s + "0)"
        if self.magnitude() == 1.0 and self.w == 1:
            return s + "1)"
        if self.vector().magnitude() == 0.0:
            return f"{s}{self.w})"
        else:
            return s + f"{self.w:.1f} + {self.vector()})"
    ...
    def __eq__(self, v):
        return self.w == v.w and self.x == v.x and self.y == v.y and self.z == v.z
    ...
    def dot(self, v):
        self.w * v.w + self.vector().dot(v.vector())

    def angle(self, v):
        if not isinstance(v, Quaternion):
            raise TypeError
        z = self.conjugate() * v
    ...
    @staticmethod
    def run_tests():
        a, b = Quaternion(1, 2, 3, 4), Quaternion(4, 0, 0, 7)
        c, d = Quaternion(0, 1, 1, 0), Quaternion(0, 0, 1, 0)

        e, f, g, h = Quaternion(0, 0, 0, 1), Quaternion(0, 0, 0, 0), Quaternion(1, 0, 0, 0), Quaternion(3, 0, 0, 0)
