...
template<typename T>
class vector3d
{    // class that serves as both vectors and points
public:
  vector3d();
  ...
  static void run_tests()
  {
    ...
    std::cout << "*** asserting i.dot(j) == j.dot(k) == k.dot(i) == 0"
              << "\n";
    assert( static_cast<double>( static_cast<int>( i.dot( j ) == j.dot( k ) ) == k.dot( i ) ) == 0 );
...
template<typename T>
vector3d<T>::vector3d() : vector3d( "no_name", 3 )
{}    // 3d default dims
template<typename T>
vector3d<T>::vector3d( const std::string & name, int dims ) : name_( name ), dims_( dims )
{
  memset( data_, 0, dims_ * sizeof( double ) );
  data_[3] = 0;    // vectors have 0 at end, pts have 1
}
template<typename T>
vector3d<T>::vector3d( const std::string & name, int dims, const std::initializer_list<T> & li ) :
  vector3d( name, dims )
{
  int i = 0;
  for( T value : li )
...
template<typename T>
std::string vector3d<T>::name() const
{
  return name_;
}
template<typename T>
...
template<typename T>
void vector3d<T>::check_bounds( int i ) const
{    // 1 extra dimension for pts/vectors
  if( i > dims_ )
  {
