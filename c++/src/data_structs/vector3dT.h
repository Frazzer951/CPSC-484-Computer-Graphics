...
template<typename T> class vector3d {    // class that serves as both vectors and points
public:
  vector3d();
...
template<typename T>
vector3d<T>::vector3d( const std::string &name, int dims, const std::initializer_list<T> &li ) : vector3d( name, dims ) {
  int i = 0;
  for ( T value : li ) {
    if ( i > dims_ ) { break; }
    data_[i++] = value;
...
template<typename T> int  vector3d<T>::dims() const { return dims_; }
...
//-----------------------
template<typename T> double vector3d<T>::dot( const vector3d<T> &v ) const {
  return data_[0] * v[0] + data_[1] * v[1] + data_[2] * v[2];
}
//-----------------------
template<typename T> double vector3d<T>::mag() const { return sqrt( dot( *this ) ); }

template<typename T> double vector3d<T>::angle( const vector3d<T> &v ) const {
...
template<typename T> vector3d<T> vector3d<T>::cross( const vector3d<T> &v ) const {
  ...
  return vector3d<T>(
      name_ + " x " + v.name_, dims_,
      { data_[1] * v[2] - data_[2] * v[1], -( data_[0] * v[2] - data_[2] * v[0] ), data_[0] * v[1] - data_[1] * v[0], 0 } );
}
//-----------------------
template<typename T> vector3d<T> vector3d<T>::zero() { return vector3d<T>( "zero", 3, { 0, 0, 0, 0 } ); }
//-----------------------

template<typename T> void vector3d<T>::check_bounds( int i ) const {    // 1 extra dimension for pts/vectors
