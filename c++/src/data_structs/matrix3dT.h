...
public:
  ...
  friend std::ostream &operator<<( std::ostream &os, const matrix3d<T> &m ) {
    ...
    os << "> OR by rows...\n";
    for ( int i = 0; i < 3; ++i ) {
      for ( int j = 0; j < 3; ++j ) { os << std::setw( 3 ) << m( i, j ) << " "; }
      os << "\n";
    }
    return os << ">";
  }
...
template<typename T>
matrix3d<T>::matrix3d( const std::string &name, int dims, const std::initializer_list<T> &li ) : matrix3d( name, dims ) {
...
template<typename T> matrix3d<T> &matrix3d<T>::operator=( T array[9] ) {
  matrix3d &m = *this;
  for ( int i = 0; i < 3; ++i ) {
    for ( int j = 0; j < 3; ++i ) { m( i, j ) = array[i + j]; }
  }
  return *this;
}
template<typename T> matrix3d<T> &matrix3d<T>::operator=( T k ) { return operator=( { k, k, k, k, k, k, k, k, k } ); }
...
template<typename T> void         matrix3d<T>::dims( const int &dims ) { dims_ = dims; }
template<typename T> int          matrix3d<T>::dims() const { return dims_; }
//=================================== LINEAR ALGEBRA ================================
...
template<typename T> matrix3d<T> &matrix3d<T>::operator-=( T k ) { return operator+=( -k ); }
template<typename T> matrix3d<T> &matrix3d<T>::operator*=( T k ) {
  matrix3d<T> &a = *this;
  name_          = std::to_string( k ) + "+" + name_;
  for ( int i = 0; i < 4; ++i ) { a[i] *= k; }
...
template<typename T> matrix3d<T> matrix3d<T>::cofactor() const {
  ...
  for ( int i = 0; i < dims_; i++ ) {
    for ( int j = 0; j < dims_; j++ ) { cof[i][j] = std::pow( -1, ( i + 1 ) + ( j + 1 ) ) * m[i][j]; }
  }

  return cof;
}
template<typename T> matrix3d<T> matrix3d<T>::adjoint() const { return cofactor().transpose(); }
template<typename T> matrix3d<T> matrix3d<T>::inverse() const { return adjoint() / determinant(); }
//=================================================================================================
template<typename T> matrix3d<T> matrix3d<T>::identity( int dims ) {
  matrix3d<T> iden( "I" + std::to_string( dims ), dims );

  for ( int i = 0; i < dims; i++ ) { iden[i][i] = 1; }
...
template<typename T> bool matrix3d<T>::operator==( const matrix3d<T> &b ) const {
  ...
  T                  error = T();
  for ( int i = 0; i < dims_; ++i ) {
    for ( int j = 0; j < dims_; ++j ) { error += std::abs( (double) ( a( i, j ) - b( i, j ) ) ); }
  }
  return error < epsilon_;
}
template<typename T> bool matrix3d<T>::operator!=( const matrix3d<T> &b ) const { return !( *this == b ); }
//=================================================================================================
// Matrix of minors
//=================================================================================================
