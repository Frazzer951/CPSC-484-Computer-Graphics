...
/* memory allocation */
static char *my_alloc( int, int, const char * );    // static added 4/3/2005
...
PlyFile *ply_open_for_writing( char *filename, int nelems, char **elem_names, int file_type, float *version ) {
  ...
  if ( strlen( name ) < 4 || strcmp( name + strlen( name ) - 4, ".ply" ) != 0 ) strcat( name, ".ply" );
...
void ply_header_complete( PlyFile *plyfile ) {
  ...
  default: fprintf( stderr, "ply_header_complete: bad file type = %d\n", plyfile->file_type ); exit( -1 );
  }

  /* write out the comments */

  for ( i = 0; i < plyfile->num_comments; i++ ) fprintf( fp, "comment %s\n", plyfile->comments[i] );

  /* write out object information */

  for ( i = 0; i < plyfile->num_obj_info; i++ ) fprintf( fp, "obj_info %s\n", plyfile->obj_info[i] );
...
void ply_put_comment( PlyFile *plyfile, char *comment ) {
  ...
  if ( plyfile->num_comments == 0 )
    ...
    plyfile->comments = (char **) realloc( plyfile->comments, sizeof( char * ) * ( plyfile->num_comments + 1 ) );
...
void ply_put_obj_info( PlyFile *plyfile, char *obj_info ) {
  ...
  if ( plyfile->num_obj_info == 0 )
    ...
    plyfile->obj_info = (char **) realloc( plyfile->obj_info, sizeof( char * ) * ( plyfile->num_obj_info + 1 ) );
...
PlyFile *ply_read( FILE *fp, int *nelems, char ***elem_names ) {
  ...
  if ( !words || !equal_strings( words[0], "ply" ) ) return ( NULL );

  while ( words ) {
    /* parse words */

    if ( equal_strings( words[0], "format" ) ) {
      if ( nwords != 3 ) return ( NULL );
  ...
  for ( i = 0; i < plyfile->nelems; i++ ) {
    ...
    for ( j = 0; j < elem->nprops; j++ ) elem->store_prop[j] = DONT_STORE_PROP;
...
PlyOtherProp *ply_get_other_properties( PlyFile *plyfile, char *elem_name, int offset ) {
  ...
  if ( other->nprops == 0 ) { elem->other_offset = NO_OTHER_PROPS; }
#endif
...
PlyOtherElems *ply_get_other_element( PlyFile *plyfile, char *elem_name, int elem_count ) {
  ...
  elem = find_element( plyfile, elem_name );
  if ( elem == NULL ) {
    fprintf( stderr, "ply_get_other_element: can't find element '%s'\n", elem_name );
    exit( -1 );
  }
...
void ply_put_other_elements( PlyFile *plyfile ) {
  ...
  if ( plyfile->other_elems == NULL ) return;
  ...
  for ( i = 0; i < plyfile->other_elems->num_elems; i++ ) {
    ...
    for ( j = 0; j < other->elem_count; j++ ) ply_put_element( plyfile, (void *) other->other_data[j] );
  }
}
...
void ply_get_info( PlyFile *plyfile, float *version, int *file_type ) {
  if ( plyfile == NULL ) return;
...
PlyElement *find_element( PlyFile *plyfile, const char *element ) {
  ...
  for ( i = 0; i < plyfile->nelems; i++ )
    if ( equal_strings( element, plyfile->elems[i]->name ) ) return ( plyfile->elems[i] );
...
void binary_get_element( PlyFile *plyfile, char *elem_ptr ) {
  ...
  for ( j = 0; j < elem->nprops; j++ ) {
    ...
    if ( prop->is_list ) { /* a list */
      ...
      /* get and store the number of items in the list */
      get_binary_item( fp, prop->count_external, &int_val, &uint_val, &double_val );
      if ( store_it ) {
        item = elem_data + prop->count_offset;
        store_item( item, prop->count_internal, int_val, uint_val, double_val );
      }
      ...
      if ( list_count == 0 ) {
        ...
        /* read items and store them into the array */
        for ( k = 0; k < list_count; k++ ) {
          get_binary_item( fp, prop->external_type, &int_val, &uint_val, &double_val );
          if ( store_it ) {
            store_item( item, prop->internal_type, int_val, uint_val, double_val );
            item += item_size;
          }
...
double get_item_value( char *item, int type ) {
  ...
  case PLY_DOUBLE:
    ...
    return ( double_value );
  default: fprintf( stderr, "get_item_value: bad type = %d\n", type ); exit( -1 );
  }
}
...
void write_binary_item( FILE *fp, int int_val, unsigned int uint_val, double double_val, int type ) {
  ...
  default: fprintf( stderr, "write_binary_item: bad type = %d\n", type ); exit( -1 );
  }
}
...
void write_ascii_item( FILE *fp, int int_val, unsigned int uint_val, double double_val, int type ) {
  ...
  default: fprintf( stderr, "write_ascii_item: bad type = %d\n", type ); exit( -1 );
  }
}
...
double old_write_ascii_item( FILE *fp, char *item, int type ) {
  ...
  case PLY_DOUBLE:
    ...
    fprintf( fp, "%g ", double_value );
    return ( double_value );
  default: fprintf( stderr, "old_write_ascii_item: bad type = %d\n", type ); exit( -1 );
  }
}
...
void get_stored_item( void *ptr, int type, int *int_val, unsigned int *uint_val, double *double_val ) {
  ...
  case PLY_DOUBLE:
    ...
    *uint_val   = *double_val;
    break;
  default: fprintf( stderr, "get_stored_item: bad type = %d\n", type ); exit( -1 );
  }
}
...
void get_binary_item( FILE *fp, int type, int *int_val, unsigned int *uint_val, double *double_val ) {
  ...
  case PLY_DOUBLE:
    ...
    *uint_val   = *double_val;
    break;
  default: fprintf( stderr, "get_binary_item: bad type = %d\n", type ); exit( -1 );
  }
}
...
void get_ascii_item( char *word, int type, int *int_val, unsigned int *uint_val, double *double_val ) {
  ...
  default: fprintf( stderr, "get_ascii_item: bad type = %d\n", type ); exit( -1 );
  }
}
...
void store_item( char *item, int type, int int_val, unsigned int uint_val, double double_val ) {
  ...
  case PLY_DOUBLE:
    ...
    *pdouble = double_val;
    break;
  default: fprintf( stderr, "store_item: bad type = %d\n", type ); exit( -1 );
  }
}
...
void add_element( PlyFile *plyfile, char **words, int ) {
  ...
  if ( plyfile->nelems == 0 )
    ...
    plyfile->elems = (PlyElement **) realloc( plyfile->elems, sizeof( PlyElement * ) * ( plyfile->nelems + 1 ) );
...
int get_prop_type( char *type_name ) {
  ...
  for ( i = PLY_START_TYPE + 1; i < PLY_END_TYPE; i++ ) {
    if ( equal_strings( type_name, type_names[i] ) ) { return ( i ); }
  }
...
char *strdup( const char *s ) {
  int   len = strlen( s );
  char *r   = (char *) malloc( len + 1 );    // KS: explicit cast to char* added 4/2/2005
  strcpy( r, s );
