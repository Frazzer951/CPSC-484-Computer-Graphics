...
Image::Image( void ) : hres( 100 ), vres( 100 ), pixels() {}

Image::Image( const Image &other ) : Image() { copy( other ); }
...
void Image::read_ppm_file( const char *file_name ) {
  ...
  if ( fscanf( file, "P%c\n", &ppm_type ) != 1 ) { cout << "Invalid PPM signature" << endl; }

  // only binary PPM supported

  if ( ppm_type != '6' ) { cout << "Only binary PPM supported" << endl; }
  ...
  if ( fscanf( file, "%d %d\n", &hres, &vres ) != 2 ) { cout << "Invalid image size" << endl; }
  ...
  if ( fscanf( file, "%d\n", &max_value ) != 1 ) { cout << "Invalid max value" << endl; }
  ...
  for ( int y = 0; y < vres; y++ ) {
    for ( int x = 0; x < hres; x++ ) {
      ...
      if ( fscanf( file, "%c%c%c", &red, &green, &blue ) != 3 ) { cout << "Invalid image" << endl; }
