...
PlyFile *ply_open_for_writing( char *filename, int nelems, char **elem_names, int file_type, float *version ) {
  ...
  ++elem;    // to silence unused warning
  char *name;
  FILE *fp;
...
PlyFile *ply_read( FILE *fp, int *nelems, char ***elem_names ) {
  ...
  char   **words;
  int      found_format = 0;
  ++found_format;    // to silence unused warning
  char      **elist;
  PlyElement *elem;
  char       *orig_line;
...
void setup_other_props( PlyFile *plyfile, PlyElement *elem ) {
  int i;
  plyfile = NULL;
  ++plyfile;    // silence unused warning
  PlyProperty *prop;
  int          size = 0;
  int          type_size;
...
PlyOtherElems *ply_get_other_element( PlyFile *plyfile, char *elem_name, int elem_count ) {
  ...
  ++num_elems;    // to silence unused warning
...
void ply_free_other_elements( PlyOtherElems *other_elems ) {
  ++other_elems;    // to silence unused warning
}
...
void add_element( PlyFile *plyfile, char **words, int nwords ) {
  PlyElement *elem;
  ++nwords;    // to silence unused warning
