...
void print_timer_now( QElapsedTimer &qet ) {
  QMutexLocker locker( &mutex );
  qDebug() << "Timer: " << qet.elapsed();
}
...
void pixels_rendered( int pixelsRendered ) {
  QMutexLocker locker( &mutex );
  qDebug() << "PIXELS RENDERED: " << pixelsRendered;
}
...
public:
  RenderNewPixelEvent( RenderPixel *px ) : QEvent( (QEvent::Type) renderNewPixelEvent_ID ), pixel( px ) {}
...
///********************* RenderThread *******************************************/
///******************************************************************************/
RenderThread::RenderThread( RenderCanvas *c, World *w ) : world( w ), canvas( c ), timer( new QElapsedTimer() ) {
...
RenderCanvas::RenderCanvas( QWindow * ) :
    w( nullptr ), thread( nullptr ), running( false ), totalTimeRendering( 0L ), updateTimer( new QTimer( this ) ),
...
void RenderCanvas::setPixmap( QPixmap qpixmap ) {
  //    qDebug() << __PRETTY_FUNCTION__;
  if ( !pixmap.isNull() ) { pixmap = QPixmap(); }
  pixmap = qpixmap;
  update();
}
...
void RenderCanvas::onRenderPause() {
  ...
  if ( !updateTimer.isActive() ) {
    qDebug() << "updateTimer is not active... returning...\n";
    return;
  }
  updateTimer.stop();
