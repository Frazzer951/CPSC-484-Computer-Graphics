...
SV_Matte::SV_Matte( void ) : Material(), ambient_brdf( new SV_Lambertian ), diffuse_brdf( new SV_Lambertian ) {}

// ---------------------------------------------------------------- copy constructor
SV_Matte::SV_Matte( const SV_Matte &other ) : SV_Matte() { copy( other ); }
...
RGBColor SV_Matte::shade( ShadeRec &sr ) {
  ...
  for ( int j = 0; j < num_lights; j++ ) {
    ...
    if ( ndotwi > 0.0 && ndotwo > 0.0 ) {
      ...
      if ( !in_shadow ) L += diffuse_brdf->f( sr, wo, wi ) * light_ptr->L( sr ) * light_ptr->G( sr ) * ndotwi;
    }
  }
...
RGBColor SV_Matte::area_light_shade( ShadeRec &sr ) {
  ...
  for ( int j = 0; j < num_lights; j++ ) {
    ...
    if ( ndotwi > 0.0 && ndotwo > 0.0 ) {
      ...
      if ( !in_shadow )
        L += diffuse_brdf->f( sr, wo, wi ) * sr.w.lights[j]->L( sr ) * sr.w.lights[j]->G( sr ) * ndotwi
