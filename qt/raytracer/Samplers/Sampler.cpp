...
Sampler::Sampler( const Sampler &s ) :
    num_samples( s.num_samples ), num_sets( s.num_sets ), samples( s.samples ), shuffled_indices( s.shuffled_indices ),
    disk_samples( s.disk_samples ), hemisphere_samples( s.hemisphere_samples ), sphere_samples( s.sphere_samples ),
...
// ------------------------------------------------------------------- get_num_samples
int  Sampler::get_num_samples( void ) { return ( num_samples ); }
...
void Sampler::setup_shuffled_indices( void ) {
  ...
  std::mt19937       g( rd() );    // uses MersenneTwister19937 pseudo-random generator
#endif
  ...
  for ( int p = 0; p < num_sets; p++ ) {
    ...
    for ( int j = 0; j < num_samples; j++ ) { shuffled_indices.push_back( indices[j] ); }
  }
}
...
Point2D Sampler::sample_unit_square( void ) {
  ...
  return ( samples[jump + shuffled_indices[jump + count++ % num_samples]] );    // 5.13
  //    return (samples[jump + count++ % num_samples]);  // Listing 5.8
}
/*
// ------------------------------------------------------------------- sample_unit_square
...
Point2D Sampler::sample_one_set( void ) { return ( samples[count++ % num_samples] ); }
