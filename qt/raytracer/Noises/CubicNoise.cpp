...
//------------------------------------------------------------------------- default constructor
CubicNoise::CubicNoise( void ) : LatticeNoise() {}

//------------------------------------------------------------------------- constructor
CubicNoise::CubicNoise( int octaves ) : LatticeNoise( octaves ) {}

//------------------------------------------------------------------------- constructor
CubicNoise::CubicNoise( int octaves, float lacunarity, float gain ) : LatticeNoise( octaves, lacunarity, gain ) {}

//------------------------------------------------------------------------- copy constructor
CubicNoise::CubicNoise( const CubicNoise &cns ) : LatticeNoise( cns ) {}
...
//------------------------------------------------------------------------- clone
CubicNoise *CubicNoise::clone( void ) const { return ( new CubicNoise ); }
...
float CubicNoise::value_noise( const Point3D &p ) const {
  ...
  for ( int k = -1; k <= 2; k++ ) {
    for ( int j = -1; j <= 2; j++ ) {
      for ( int i = -1; i <= 2; i++ ) { xknots[i + 1] = value_table[INDEX( ix + i, iy + j, iz + k )]; }
...
Vector3D CubicNoise::vector_noise( const Point3D &p ) const {
  ...
  for ( int k = -1; k <= 2; k++ ) {
    for ( int j = -1; j <= 2; j++ ) {
      for ( int i = -1; i <= 2; i++ ) { xknots[i + 1] = vector_table[INDEX( ix + i, iy + j, iz + k )]; }
      yknots[j + 1] = four_knot_spline( fx, xknots );
    }
    zknots[k + 1] = four_knot_spline( fy, yknots );
  }
