...
//------------------------------------------------------------------------- default constructor
LinearNoise::LinearNoise( void ) : LatticeNoise() {}

LinearNoise::LinearNoise( int octaves, float lacunarity, float gain ) : LatticeNoise( octaves, lacunarity, gain ) {}

//------------------------------------------------------------------------- copy constructor
LinearNoise::LinearNoise( const LinearNoise &lns ) : LatticeNoise( lns ) {}

//------------------------------------------------------------------------- clone
LinearNoise *LinearNoise::clone( void ) const { return ( new LinearNoise ); }
...
float LinearNoise::value_noise( const Point3D &p ) const {
  ...
  for ( int k = 0; k <= 1; k++ )
    for ( int j = 0; j <= 1; j++ )
      for ( int i = 0; i <= 1; i++ ) d[k][j][i] = value_table[INDEX( ix + i, iy + j, iz + k )];
...
Vector3D LinearNoise::vector_noise( const Point3D &p ) const {
  ...
  for ( int k = 0; k <= 1; k++ ) {
    for ( int j = 0; j <= 1; j++ ) {
      for ( int i = 0; i <= 1; i++ ) { d[k][j][i] = vector_table[INDEX( ix + i, iy + j, iz + k )]; }
    }
  }
