...
//---------------------------------------------------------------------------------------- default constructor
LatticeNoise::LatticeNoise( void ) : num_octaves( 1 ), lacunarity( 2.0 ), gain( 0.5 ) {
...
LatticeNoise::LatticeNoise( int _num_octaves ) : num_octaves( _num_octaves ), lacunarity( 2.0 ), gain( 0.5 ) {
  init_value_table( seed_value );
  init_vector_table( seed_value );
  compute_fbm_bounds();
}
...
LatticeNoise::LatticeNoise( int _num_octaves, float _lacunarity, float _gain ) :
    num_octaves( _num_octaves ), lacunarity( _lacunarity ), gain( _gain ) {
  init_value_table( seed_value );
  init_vector_table( seed_value );
  compute_fbm_bounds();
}
...
LatticeNoise::LatticeNoise( const LatticeNoise &ln ) :
    num_octaves( ln.num_octaves ), lacunarity( ln.lacunarity ), gain( ln.gain ) {
  init_value_table( seed_value );
  init_vector_table( seed_value );
  compute_fbm_bounds();
}
...
void LatticeNoise::init_value_table( int seed_value ) {
  ...
  for ( int i = 0; i < kTableSize; i++ ) value_table[i] = 1.0 - 2.0 * rand_float();    // In the range [-1, +1]
}
...
void LatticeNoise::set_lacunarity( float _lacunarity ) { lacunarity = _lacunarity; }
