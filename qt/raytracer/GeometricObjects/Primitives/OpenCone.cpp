...
OpenCone::OpenCone( void ) : GeometricObject(), height( 2.0 ), radius( 1.0 ), hr( 2.0 ), h2r2( 4.0 ) {}

OpenCone::OpenCone( double h, double r ) :
    GeometricObject(), height( h ), radius( r ), hr( height / radius ), h2r2( ( height * height ) / ( radius * radius ) ) {}

OpenCone::OpenCone( const OpenCone &openCone ) :
    GeometricObject( openCone ), height( openCone.height ), radius( openCone.radius ), hr( openCone.hr ),
    h2r2( openCone.h2r2 ) {}

OpenCone *OpenCone::clone( void ) const { return new OpenCone( *this ); }
  ...
  if ( disc < 0.0 )
    ...
    if ( t > kEpsilon ) {
      ...
      if ( 0 <= yhit && yhit <= height ) {
        ...
        if ( -ray.d.dot( sr.normal ) < 0.0 ) sr.normal = -sr.normal;
...
bool OpenCone::shadow_hit( const Ray &ray, double &tmin ) const {
