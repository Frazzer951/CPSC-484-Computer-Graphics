...
Disk::Disk( void ) : GeometricObject(), center( 0.0, 0.0, 0.0 ), normal( 0.0, 1.0, 0.0 ), radius( 1.0 ), r_squared( 1.0 ) {}

Disk::Disk( const Point3D &c, const Normal &n, double r ) :
    GeometricObject(), center( c ), normal( n ), radius( r ), r_squared( radius * radius ) {
  normal.normalize();
}

Disk::Disk( const Disk &disk ) :
    GeometricObject( disk ), center( disk.center ), normal( disk.normal ), radius( disk.radius ), r_squared( disk.r_squared ) {
}
