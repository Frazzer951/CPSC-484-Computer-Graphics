...
ConcaveCylinder::ConcaveCylinder( void ) : GeometricObject(), y0( -1.0 ), y1( 1.0 ), radius( 1.0 ), inv_radius( 1.0 ) {}

// ---------------------------------------------------------------- constructor

ConcaveCylinder::ConcaveCylinder( const double bottom, const double top, const double r ) :
    GeometricObject(), y0( bottom ), y1( top ), radius( r ), inv_radius( 1.0 / radius ) {}

// ---------------------------------------------------------------- copy constructor

ConcaveCylinder::ConcaveCylinder( const ConcaveCylinder &c ) :
    GeometricObject( c ), y0( c.y0 ), y1( c.y1 ), radius( c.radius ), inv_radius( c.inv_radius ) {}

// ---------------------------------------------------------------- clone

ConcaveCylinder *ConcaveCylinder::clone( void ) const { return new ConcaveCylinder( *this ); }
...
// ----------------------------------------------------------------------------- hit
// The normal always points inwards
...
bool ConcaveCylinder::shadow_hit( const Ray &ray, double &tmin ) const {
