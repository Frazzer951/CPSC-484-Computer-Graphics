...
ConcavePartSphere::ConcavePartSphere( void ) :
    GeometricObject(), center( 0.0 ), radius( 1.0 ), phi_min( 0.0 ),    // in radians
    phi_max( TWO_PI ),                                                  // in radians
    theta_min( 0.0 ),                                                   // in radians measured from top
    theta_max( PI ),                                                    // in radians measured from top
    cos_theta_min( 1.0 ), cos_theta_max( -1.0 ) {}

// ---------------------------------------------------------------- donstructor

ConcavePartSphere::ConcavePartSphere( const Point3D c, const double r,
    ...
    cos_theta_min( cos( theta_min ) ), cos_theta_max( cos( theta_max ) ) {}

// ---------------------------------------------------------------- constructor

ConcavePartSphere::ConcavePartSphere( const Point3D c, const double r ) :
    GeometricObject(), center( c ), radius( r ), phi_min( 0.0 ), phi_max( TWO_PI ), theta_min( 0.0 ), theta_max( PI ),
    cos_theta_min( 1.0 ), cos_theta_max( -1.0 ) {}

// ---------------------------------------------------------------- clone

ConcavePartSphere *ConcavePartSphere::clone( void ) const { return new ConcavePartSphere( *this ); }

// ---------------------------------------------------------------- copy constructor

ConcavePartSphere::ConcavePartSphere( const ConcavePartSphere &ps ) :
    GeometricObject( ps ), center( ps.center ), radius( ps.radius ), phi_min( ps.phi_min ), phi_max( ps.phi_max ),
    theta_min( ps.theta_min ), theta_max( ps.theta_max ), cos_theta_min( ps.cos_theta_min ),
    cos_theta_max( ps.cos_theta_max ) {}

// ---------------------------------------------------------------- assignment operator

ConcavePartSphere &ConcavePartSphere::operator=( const ConcavePartSphere &rhs ) {
...
bool ConcavePartSphere::shadow_hit( const Ray &ray, double &tmin ) const {
