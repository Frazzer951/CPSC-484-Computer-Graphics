...
PartAnnulus::PartAnnulus( void ) :
    GeometricObject(), y( 0.0 ), inner_radius( 0.8 ), outer_radius( 1.0 ), phi_min( 0.0 ), phi_max( TWO_PI ),
    i_r_squared( 0.64 ), o_r_squared( 1.0 ), normal_up( true ) {}
...
PartAnnulus::PartAnnulus( const double y, double i_r, double o_r,
                          ...
                          bool         up ) :
    GeometricObject(),
    y( y ), inner_radius( i_r ), outer_radius( o_r ), phi_min( azimuth_min * PI_ON_180 ), phi_max( azimuth_max * PI_ON_180 ),
    i_r_squared( inner_radius * inner_radius ), o_r_squared( outer_radius * outer_radius ), normal_up( up ) {}

PartAnnulus::PartAnnulus( const PartAnnulus &partAnnulus ) :
    GeometricObject( partAnnulus ), y( partAnnulus.y ), inner_radius( partAnnulus.inner_radius ),
    outer_radius( partAnnulus.outer_radius ), phi_min( partAnnulus.phi_min ), phi_max( partAnnulus.phi_max ),
    i_r_squared( partAnnulus.i_r_squared ), o_r_squared( partAnnulus.o_r_squared ), normal_up( partAnnulus.normal_up ) {}

PartAnnulus *PartAnnulus::clone( void ) const { return new PartAnnulus( *this ); }
...
PartAnnulus::~PartAnnulus( void ) { /*  delete material_ptr; */
}
