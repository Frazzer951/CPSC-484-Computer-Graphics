#include "GeometricObjects/PartObjects/ConcavePartTorus.h"
...
// ----------------------------------------------------------------  default constructor
// a default ConcavePartTorus is a whole torus

ConcavePartTorus::ConcavePartTorus( void ) :
    GeometricObject(), a( 2.0 ), b( 0.5 ), phi_min( 0.0 ),    // in radians
...
ConcavePartTorus::ConcavePartTorus( const double i_r, const double o_r,
    ...
    GeometricObject(),
    a( i_r ), b( o_r ), phi_min( azimuth_min * PI_ON_180 ),    // in radians
  ...
  if ( theta_min < 0 ) theta_min += TWO_PI;
}

// ---------------------------------------------------------------- clone

ConcavePartTorus *ConcavePartTorus::clone( void ) const { return new ConcavePartTorus( *this ); }

// ---------------------------------------------------------------- copy constructor

ConcavePartTorus::ConcavePartTorus( const ConcavePartTorus &ps ) :
    GeometricObject( ps ), a( ps.a ), b( ps.b ), phi_min( ps.phi_min ), phi_max( ps.phi_max ), theta_min( ps.theta_min ),
    theta_max( ps.theta_max ), bbox( ps.bbox ) {}

// ---------------------------------------------------------------- assignment operator

ConcavePartTorus &ConcavePartTorus::operator=( const ConcavePartTorus &rhs ) {
...
Normal ConcavePartTorus::compute_normal( const Point3D &p ) const {
  Normal normal;
    ...
    if ( roots[j] > kEpsilon ) {
      if ( roots[j] < t ) {
        ...
        if ( theta_max < theta_min ) {
          good_theta = ( ( theta_min <= theta && theta <= 360 ) || ( 0 <= theta && theta <= theta_max ) );
        } else {
...
bool ConcavePartTorus::shadow_hit( const Ray &ray, double &tmin ) const {
