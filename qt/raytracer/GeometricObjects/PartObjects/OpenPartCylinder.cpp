...
// ----------------------------------------------------------------  default constructor
// a default OpenPartCylinder is a whole cylinder

OpenPartCylinder::OpenPartCylinder( void ) :
    GeometricObject(), y0( -1.0 ), y1( 1.0 ), radius( 1.0 ), phi_min( 0.0 ),    // in radians
    phi_max( TWO_PI ),                                                          // in radians
    inv_radius( 1.0 ) {}

// ---------------------------------------------------------------- constructor

OpenPartCylinder::OpenPartCylinder( const double bottom, const double top, const double radius,
                                    const double azimuth_min,       // in degrees
                                    const double azimuth_max ) :    // in degrees

    GeometricObject(),
    y0( bottom ), y1( top ), radius( radius ), phi_min( azimuth_min * PI_ON_180 ),    // in radians
...
OpenPartCylinder::OpenPartCylinder( const double bottom, const double top, const double r ) :
    GeometricObject(), y0( bottom ), y1( top ), radius( r ), phi_min( 0.0 ), phi_max( TWO_PI ), inv_radius( 1.0 / radius ) {}

// ---------------------------------------------------------------- clone

OpenPartCylinder *OpenPartCylinder::clone( void ) const { return new OpenPartCylinder( *this ); }

// ---------------------------------------------------------------- copy constructor

OpenPartCylinder::OpenPartCylinder( const OpenPartCylinder &ps ) :
    GeometricObject( ps ), y0( ps.y0 ), y1( ps.y1 ), radius( ps.radius ), phi_min( ps.phi_min ), phi_max( ps.phi_max ),
...
bool OpenPartCylinder::shadow_hit( const Ray &ray, double &tmin ) const {
