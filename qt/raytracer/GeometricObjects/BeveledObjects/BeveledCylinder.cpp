...
BeveledCylinder::BeveledCylinder( void ) :
    Compound(), y0( -1.0 ), y1( 1.0 ), r( 1.0 ), rb( 0.1 ), bbox( Point3D( y0 ), Point3D( y1 ) ) {
  // bottom
...
BeveledCylinder::BeveledCylinder( const double bottom, const double top, const double radius, const double bevel_radius ) :
    Compound(), y0( bottom ), y1( top ), r( radius ), rb( bevel_radius ), bbox( Point3D( y0 ), Point3D( y1 ) ) {
  // bottom
  objects.push_back( new Disk( Point3D( 0.0, y0, 0.0 ), Normal( 0.0, -1.0, 0.0 ), r - 2 * rb ) );
...
BeveledCylinder::BeveledCylinder( const BeveledCylinder &bc ) :
    Compound( bc ), y0( bc.y0 ), y1( bc.y1 ), r( bc.r ), rb( bc.rb ), bbox( bc.bbox ) {}

BeveledCylinder *BeveledCylinder::clone( void ) const { return new BeveledCylinder( *this ); }
...
BBox BeveledCylinder::get_bounding_box( void ) const { return bbox; }
...
bool BeveledCylinder::shadow_hit( const Ray &ray, double &tmin ) const {
