...
BeveledBox::BeveledBox( void ) : Compound(), p0( -1.0 ), p1( 1.0 ), rb( 0.1 ), bbox( p0, p1 ) {
  // edges
  ...
  // top edges  (+ve y)
  Instance *top_front_edge =
      new Instance( new OpenCylinder( -( p1.x - p0.x - 2 * rb ) / 2, ( p1.x - p0.x - 2 * rb ) / 2, rb ) );    // top front edge
  ...
  //	// top back (-ve z)
  Instance *top_back_edge =
      new Instance( new OpenCylinder( -( p1.x - p0.x - 2 * rb ) / 2, ( p1.x - p0.x - 2 * rb ) / 2, rb ) );    // top back edge
  ...
  //	// top right (+ve x)
  Instance *top_right_edge =
      new Instance( new OpenCylinder( -( p1.z - p0.z - 2 * rb ) / 2, ( p1.z - p0.z - 2 * rb ) / 2, rb ) );    // top right edge
  ...
  //	// top left (-ve x)
  Instance *top_left_edge =
      new Instance( new OpenCylinder( -( p1.z - p0.z - 2 * rb ) / 2, ( p1.z - p0.z - 2 * rb ) / 2, rb ) );    // top left edge
  ...
  //	// bottom edges  (-ve y)
  //	// bottom front  (+ve z)
  Instance *bottom_front_edge = new Instance(
      new OpenCylinder( -( p1.x - p0.x - 2 * rb ) / 2, ( p1.x - p0.x - 2 * rb ) / 2, rb ) );    // bottom fromt edge
  ...
  //	// bottom back  (-ve z)
  Instance *bottom_back_edge = new Instance(
      new OpenCylinder( -( p1.x - p0.x - 2 * rb ) / 2, ( p1.x - p0.x - 2 * rb ) / 2, rb ) );    // bottom back edge
  ...
  Instance *bottom_right_edge = new Instance(
      new OpenCylinder( -( p1.z - p0.z - 2 * rb ) / 2, ( p1.z - p0.z - 2 * rb ) / 2, rb ) );    // bottom right edge
  ...
  Instance *bottom_left_edge = new Instance(
      new OpenCylinder( -( p1.z - p0.z - 2 * rb ) / 2, ( p1.z - p0.z - 2 * rb ) / 2, rb ) );    // bottom left edge
...
BeveledBox::BeveledBox( const Point3D &min_corner, const Point3D &max_corner, const double bevel_radius ) :
    Compound(), p0( min_corner ), p1( max_corner ), rb( bevel_radius ), bbox( p0, p1 ) {
  // edges
  // since the cylinders have to be defined initially in the vertical direction, it's easiest to use -(...)/2, +(...)/2 for
  // y0 and y1 in the constructors, and then rotate them about their centers.
...
BBox BeveledBox::get_bounding_box( void ) { return ( bbox ); }
