...
Instance::Instance( void ) : GeometricObject(), object_ptr( NULL ), inv_matrix(), bbox(), transform_the_texture( true ) {
  forward_matrix.set_identity();
}
// ----------------------------------------------------------------  constructor

Instance::Instance( GeometricObject *obj_ptr ) :
    GeometricObject(), object_ptr( obj_ptr ), inv_matrix(), bbox(), transform_the_texture( true ) {
  forward_matrix.set_identity();
}
// ---------------------------------------------------------------- copy constructor

Instance::Instance( const Instance &instance ) :
    GeometricObject( instance ), inv_matrix( instance.inv_matrix ), transform_the_texture( instance.transform_the_texture ) {
...
bool Instance::hit( const Ray &ray, double &t, ShadeRec &sr ) const {
  ...
  if ( object_ptr->hit( inv_ray, t, sr ) ) {
    ...
    if ( !transform_the_texture ) { sr.local_hit_point = ray.o + t * ray.d; }
    return ( true );
  }
  return ( false );
}
...
void Instance::shear( const Matrix &s ) {
  ...
  // discriminant
  double d = 1.0 - s.m[1][0] * s.m[0][1] - s.m[2][0] * s.m[0][2] - s.m[2][1] * s.m[1][2] + s.m[1][0] * s.m[2][1] * s.m[0][2]
