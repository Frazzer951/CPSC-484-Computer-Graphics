...
RoundRimmedBowl::RoundRimmedBowl( void ) : Compound() {
...
RoundRimmedBowl::RoundRimmedBowl( double i_r, double o_r, const Point3D &c ) : Compound() {
...
RoundRimmedBowl::RoundRimmedBowl( const RoundRimmedBowl &b ) : Compound( b ) {}

RoundRimmedBowl *RoundRimmedBowl::clone( void ) const { return new RoundRimmedBowl( *this ); }
...
bool RoundRimmedBowl::hit( const Ray &ray, double &tmin, ShadeRec &sr ) const { return Compound::hit( ray, tmin, sr ); }

bool RoundRimmedBowl::shadow_hit( const Ray &ray, double &tmin ) const {
