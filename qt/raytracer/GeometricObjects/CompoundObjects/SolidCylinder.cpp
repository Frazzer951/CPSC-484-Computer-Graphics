...
SolidCylinder::SolidCylinder( void ) : Compound() {
  objects.push_back( new OpenCylinder );
  objects.push_back( new Disk( Point3D( 0.0, -1.0, 0.0 ), Normal( 0.0, -1.0, 0.0 ), 1.0 ) );
  objects.push_back( new Disk( Point3D( 0.0, 1.0, 0.0 ), Normal( 0.0, 1.0, 0.0 ), 1.0 ) );
}

SolidCylinder::SolidCylinder( const double bottom, const double top, const double radius ) : Compound() {
...
SolidCylinder::SolidCylinder( const SolidCylinder &sc ) : Compound( sc ) {}

SolidCylinder *SolidCylinder::clone( void ) const { return new SolidCylinder( *this ); }
...
bool SolidCylinder::hit( const Ray &ray, double &tmin, ShadeRec &sr ) const { return Compound::hit( ray, tmin, sr ); }
