...
#include "GeometricObjects/Triangles/MeshTriangle.h"
...
Grid *Grid::clone( void ) const { return ( new Grid( *this ) ); }
// ---------------------------------------------------------------- copy constructor
// not implemented
...
Grid &Grid::operator=( const Grid & ) { return *this; }
// ---------------------------------------------------------------- destructor
// not implemented

Grid::~Grid( void ) {}

BBox Grid::get_bounding_box( void ) { return ( bbox ); }
//------------------------------------------------------------------ setup_cells
...
void Grid::setup_cells( void ) {
  ...
  for ( int j = 0; j < num_cells; j++ ) cells.push_back( NULL );
  ...
  for ( int j = 0; j < num_cells; j++ ) counts.push_back( 0 );
  ...
  for ( int j = 0; j < num_cells; j++ ) {
    if ( counts[j] == 0 ) num_zeroes += 1;
    if ( counts[j] == 1 ) num_ones += 1;
    if ( counts[j] == 2 ) num_twos += 1;
    if ( counts[j] == 3 ) num_threes += 1;
    if ( counts[j] > 3 ) num_greater += 1;
  }
...
Point3D Grid::find_min_bounds( void ) {
  ...
  for ( int j = 0; j < num_objects; j++ ) {
    ...
    if ( object_box.x0 < p0.x ) p0.x = object_box.x0;
    if ( object_box.y0 < p0.y ) p0.y = object_box.y0;
    if ( object_box.z0 < p0.z ) p0.z = object_box.z0;
  }
...
Point3D Grid::find_max_bounds( void ) {
  ...
  for ( int j = 0; j < num_objects; j++ ) {
    ...
    if ( object_box.x1 > p1.x ) p1.x = object_box.x1;
    if ( object_box.y1 > p1.y ) p1.y = object_box.y1;
    if ( object_box.z1 > p1.z ) p1.z = object_box.z1;
  }
...
void Grid::read_ply_file( char *file_name, const int triangle_type ) {
  ...
  plyfile = ply_open_for_reading( file_name, &nelems, &elist, &file_type, &version );
  ...
  for ( i = 0; i < nelems; i++ ) {    // there are only two elements in our files: vertices and faces
    ...
    if ( equal_strings( "vertex", elem_name ) ) {
      ...
      for ( j = 0; j < num_elems; j++ ) {
        ...
        ply_get_element( plyfile, (void *) vertex_ptr );
    ...
    if ( equal_strings( "face", elem_name ) ) {
      ...
      ply_get_property( plyfile, elem_name, &face_props[0] );    // only one property - a list
      ...
      for ( j = 0; j < num_elems; j++ ) {
        ...
        ply_get_element( plyfile, (void *) face_ptr );

        // construct a mesh triangle of the specified type

        if ( triangle_type == flat ) {
          FlatMeshTriangle *triangle_ptr =
              new FlatMeshTriangle( mesh_ptr, face_ptr->verts[0], face_ptr->verts[1], face_ptr->verts[2] );
        ...
        if ( triangle_type == smooth ) {
          SmoothMeshTriangle *triangle_ptr =
              new SmoothMeshTriangle( mesh_ptr, face_ptr->verts[0], face_ptr->verts[1], face_ptr->verts[2] );
          triangle_ptr->compute_normal(
              reverse_normal );    // the "flat triangle" normal is used to compute the average normal at each mesh vertex
          objects.push_back(
              triangle_ptr );    // it's quicker to do it once here, than have to do it on average 6 times in compute_mesh_normals
    ...
    for ( j = 0; j < nprops; j++ ) printf( "property %s\n", plist[j]->name );

  }    // end of for (i = 0; i < nelems; i++)

  // grab and print out the comments in the file

  comments = ply_get_comments( plyfile, &num_comments );

  for ( i = 0; i < num_comments; i++ ) printf( "comment = '%s'\n", comments[i] );

  // grab and print out the object information

  obj_info = ply_get_obj_info( plyfile, &num_obj_info );

  for ( i = 0; i < num_obj_info; i++ ) printf( "obj_info = '%s'\n", obj_info[i] );
...
void Grid::tessellate_flat_sphere( const int horizontal_steps, const int vertical_steps ) {
  ...
  for ( int k = 1; k <= vertical_steps - 2; k++ ) {
    for ( int j = 0; j <= horizontal_steps - 1; j++ ) {
      ...
      Point3D v0( sin( 2.0 * pi * j / horizontal_steps )
      ...
      Point3D v1( sin( 2.0 * pi * ( j + 1 ) / horizontal_steps )
      ...
      // vertices
      v0 = Point3D(
          sin( 2.0 * pi * ( j + 1 ) / horizontal_steps ) * sin( pi * k / vertical_steps ),    // top right, use k, j + 1
      ...
      v2 = Point3D( sin( 2.0 * pi * ( j + 1 ) / horizontal_steps )
                        * sin( pi * ( k + 1 ) / vertical_steps ),    // bottom  right, use k + 1, j + 1
                    cos( pi * ( k + 1 ) / vertical_steps ),
                    cos( 2.0 * pi * ( j + 1 ) / horizontal_steps ) * sin( pi * ( k + 1 ) / vertical_steps ) );
...
bool Grid::hit( const Ray &ray, double &t, ShadeRec &sr ) const {
  ...
  double x0 = bbox.x0, y0 = bbox.y0, z0 = bbox.z0;
  double x1 = bbox.x1, y1 = bbox.y1, z1 = bbox.z1;
