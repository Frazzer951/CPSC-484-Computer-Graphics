...
public:
  Archway( double width, double height, double depth, double column_width, double num_blocks, double num_wedges, double rb );
  Archway( const Archway &aw );

  virtual Archway *clone( void ) const;
  virtual Archway &operator=( const Archway &rhs );

  virtual ~Archway( void );

  void construct_archway( void );
