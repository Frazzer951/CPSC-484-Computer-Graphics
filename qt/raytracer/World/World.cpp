...
Sphere*
World::sphere_maker(Point3D pt, float radius, RGBColor color, float ka, float kd) {
    Matte* matte_ptr = new Matte();
    matte_ptr->set_ka(ka);
    matte_ptr->set_kd(kd);
    matte_ptr->set_cd(color);
    Sphere*	sphere_ptr = new Sphere(pt, radius);
...
World::build() {
    ...
    // spheres
    add_object(sphere_maker(Point3D(5, 3, 0), 30, yellow));
    add_object(sphere_maker(Point3D(45, -7, -60), 20, brown));
    add_object(sphere_maker(Point3D(40, 43, -100), 17, darkGreen));

    add_object(sphere_maker(Point3D(-20, 28, -15), 20, orange));
    add_object(sphere_maker(Point3D(-25, -7, -35), 27, green));
    add_object(sphere_maker(Point3D(20, -27, -35), 25, lightGreen));
