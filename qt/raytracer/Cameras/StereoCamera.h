...
typedef enum { parallel, transverse } ViewingType;
...
public:
  ...
  virtual void render_scene( const World &w, float offset = 0.0, int x = 0 );

private:
  void copy( const StereoCamera &other );
...
inline void StereoCamera::use_parallel_viewing() { viewing = parallel; }

inline void StereoCamera::use_transverse_viewing() { viewing = transverse; }

inline void StereoCamera::set_pixel_gap( int gap ) { pixel_gap = gap; }

inline void StereoCamera::set_stereo_angle( float stereo_angle ) { beta = stereo_angle; }
