...
// ---------------------------------------------------------------------- clone
PerfectSpecular* 
PerfectSpecular::clone(void) const { return (new PerfectSpecular(*this)); }

// ---------------------------------------------------------- f
RGBColor
PerfectSpecular::f(const ShadeRec&, const Vector3D&, const Vector3D&) const { return (black); }
// ---------------------------------------------------------- sample_f
...
PerfectSpecular::sample_f(const ShadeRec& sr, const Vector3D& wo, Vector3D& wi) const {
    ...
    return (kr * cr / fabs(sr.normal.dot(wi))); // why is this fabs? // kr would be a Fresnel term in a Fresnel reflector
...
PerfectSpecular::sample_f(const ShadeRec& sr, const Vector3D& wo, Vector3D& wi, float& pdf) const {
    float ndotwo = sr.normal.dot(wo);
    wi = -wo + 2.0 * sr.normal.dot(ndotwo);
...
RGBColor
PerfectSpecular::rho(const ShadeRec&, const Vector3D&) const {
	return (black);
}
