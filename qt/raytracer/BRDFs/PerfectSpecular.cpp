...
RGBColor PerfectSpecular::sample_f( const ShadeRec &sr, const Vector3D &wo, Vector3D &wi ) const {
  ...
  return ( kr * cr
           / fabs( sr.normal.dot( wi ) ) );    // why is this fabs? // kr would be a Fresnel term in a Fresnel reflector
...
RGBColor PerfectSpecular::rho( const ShadeRec &, const Vector3D & ) const { return ( black ); }
