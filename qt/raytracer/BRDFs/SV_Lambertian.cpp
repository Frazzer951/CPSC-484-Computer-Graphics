...
SV_Lambertian::SV_Lambertian( void ) : BRDF(), kd( 0.0 ), cd( NULL ) {}

SV_Lambertian::SV_Lambertian( const SV_Lambertian &other ) : BRDF() { copy( other ); }
...
SV_Lambertian *SV_Lambertian::clone( void ) const { return new SV_Lambertian( *this ); }

RGBColor SV_Lambertian::rho( const ShadeRec &sr, const Vector3D & /*wo*/ ) const { return kd * cd->get_color( sr ); }
