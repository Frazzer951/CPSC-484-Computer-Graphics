...
void World::init_environment(void) {
    init_cameras();
    init_viewplane();
    init_light();
    init_plane();
...
void World::checkerboard_row(RGBColor color1, RGBColor color2, int index) {
    for (int i = -7; i < 7; ++i) {
...
void World::build_checkerboard(RGBColor c1, RGBColor c2) {
    for (int i = 0; i < 10; i += 2) {
        checkerboard_row(c1, c2, i);
        checkerboard_row(c2, c1, i + 1);
    }
}
...
void World::build_city(void) {
    ...
    add_bb_helper(lightBlue,    Point3D(-4, -3, -40), Point3D(-3, -3,   40));
    add_bb_helper(grey,         Point3D( 2, -2, -40), Point3D( 3, -1,   20));
    add_bb_helper(red,          Point3D(-4,  3, -40), Point3D(-3,  4,   25));

    add_bb_helper(darkBlue,     Point3D(-6, -5, -40), Point3D(-5, -5,   40));
